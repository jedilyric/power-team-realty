      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <img src="https://images.unsplash.com/photo-1560250097-0b93528c311a?w=600&q=80" alt="Bryan Powers" className="rounded-2xl shadow-xl w-full object-cover h-96" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <span className="text-gold font-semibold tracking-wider text-sm uppercase">Lead Agent & Co-Founder</span>
            ...
            <div className="flex flex-wrap gap-3">
              {['Diamond Award Recipient', 'Multi-Million Dollar Producer', 'Top 1% Arizona Agents'].map(badge => (
                <span key={badge} className="bg-gold/10 border border-gold/30 text-gold text-xs font-semibold px-3 py-1.5 rounded-full">{badge}</span>
              ))}
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-20 px-6 bg-surface">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:order-2">
            <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=600&q=80" alt="Jamie Powers" className="rounded-2xl shadow-xl w-full object-cover h-96" />
          </motion.div>
